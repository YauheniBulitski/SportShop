...
public interface OrdersItemRepository extends CrudRepository<OrdersItem, Long> {
    ...
    BigDecimal ordersTotalPrice(@Param("order") Orders order);

    @Query("select oi.product from OrdersItem oi where oi.order=:order")
    List<Product> findAllProductByOrderId(@Param("order") Orders order);

    @Query("select oi.product from OrdersItem oi where oi.order=:order")
    List<Product> findAllProductByOrderIdPageable(@Param("order") Orders order, Pageable pageable);
}
